...
public class BaseballGame extends MouseAdapter implements Runnable {
	...
	public void run() {
		...
		for (int i = 0; i < 6; i++) {
			labels[0][i].setForeground(Color.black);
		}

		if (team == 1)
			labels[0][curInning + 1].setForeground(Color.blue);
		else
			labels[0][curInning + 1].setForeground(Color.red);
			...
			if (location == 1) {
				...
				if (hit < 3) {
					...
					Fielder newFielder = new Fielder(LEFT_FIELDER, panel,
							new Point2D.Double(leftField[hit].x / 2, leftField[hit].y / 2));
					list.add(newFielder);
					newFielder.start();
					...
					incrementOut();
					displayText = "Out!";
					Fielder newFielder = new Fielder(LEFT_FIELDER, panel, leftField[hit]);
					list.add(newFielder);
					newFielder.start();
				} else {
					displayText = "Homerun!";
				...
				if (hit < 3) {
					...
					Fielder newFielder = new Fielder(CENTER_FIELDER, panel,
							new Point2D.Double(centerField[hit].x / 2, centerField[hit].y / 2));
					list.add(newFielder);
					newFielder.start();
				} else if (hit < 6) {
					incrementOut();
					displayText = "Out!";
					Fielder newFielder = new Fielder(CENTER_FIELDER, panel, centerField[hit]);
					list.add(newFielder);
					newFielder.start();
				} else {
					displayText = "Homerun!";
				...
				if (hit < 3) {
					...
					Fielder newFielder = new Fielder(RIGHT_FIELDER, panel,
							new Point2D.Double(rightField[hit].x / 2, rightField[hit].y / 2));
					list.add(newFielder);
					newFielder.start();
					...
					incrementOut();
					displayText = "Out!";
					Fielder newFielder = new Fielder(RIGHT_FIELDER, panel, rightField[hit]);
					list.add(newFielder);
					newFielder.start();
				} else {
					displayText = "Homerun!";
